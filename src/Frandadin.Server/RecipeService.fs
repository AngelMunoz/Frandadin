...
module private Recipes =
    ...
    let find (pagination: PaginationParams) (userId: int): Async<Result<PaginationResult<Recipe>, exn>> =
        async {
            ...
            let! countQuery = defaultConnection
                              |> Sql.connect
            ...
            let! listQuery = defaultConnection
                             |> Sql.connect
            ...
            | exceptions ->
                ...
                let msg =
                    ...
                    | Error count, Error query -> count.Message + "|" + query.Message
                    | Error count, _ -> count.Message
                    | _, Error query -> query.Message
                    | _, _ -> ""
    ...
    let private findSubs (recipeId: int) =
        async {
            ...
            let! ingredientsQuery = defaultConnection
                                    |> Sql.connect
            ...
            let! stepsQuery = defaultConnection
                              |> Sql.connect
    ...
    let findRecipe (id: int) (withSubs: bool): Async<Result<Recipe, exn>> =
        async {
            let! recipeQuery = defaultConnection
                               |> Sql.connect
            ...
            | true ->
                ...
                | Result.Error recipesErr, _, _ -> return Error recipesErr
            | false ->
                match recipeQuery with
                | Result.Ok recipes -> return Result.Ok recipes.Head
                | Error ex -> return Error ex
        }

    let createRecipe
        (preRecipe: {| title: string; imageUrl: Option<string>; description: Option<string>; notes: Option<string> |})
        (userId: int)
        ...
        async {
            let! result = defaultConnection
                          |> Sql.connect
                          |> Sql.query @"INSERT INTO recipes(userid, title, imageurl, description, notes)
            ...
            match result with
            | Result.Ok rows -> return! findRecipe rows.Head true
            | Error ex -> return Error ex
        }
    ...
    let delete (recipeId: int): Async<Result<bool, exn>> =
        async {
            let! deleteRecipe = defaultConnection
                                |> Sql.connect
            ...
            | Result.Ok ->
                let! result = defaultConnection
                              ...
                              |> Sql.executeNonQueryAsync
                match result with
                | Result.Ok -> return Result.Ok true
        ...
        { find =
              ctx.Authorize <| fun pagination ->
                  async {
                      ...
                      | Error ex ->
                          ...
                          return Error
                                     { message = "Unexpected error in [find:recipes]"
                                       code = Some 422
                                       errors = List.empty }
                  }

          findOne =
              ctx.Authorize <| fun recipeId ->
                  async {
                      ...
                      | Error ex ->
                          ...
                          ctx.HttpContext.Response.StatusCode <- 422
                          return Error
                                     { message = "Unexpected error in [findOne:recipes]"
                                       code = Some 422
                                       errors = List.empty }
                  }

          create =
              ctx.Authorize <| fun payload ->
                  async {
                  ...
                  async {
                      ...
                      if recipe.userid <> userId then
                          ...
                          | Result.Ok result ->
                              ...
                              | false ->
                                  return Error
                                             { message = sprintf "Failed to update recipe %i" recipe.id
                                               code = Some 422
                                               errors = List.empty }
                          | Error ex ->
                              eprintfn "%O" ex
                              ctx.HttpContext.Response.StatusCode <- 422
                              return Error
                                         { message = "Unexpected Error in [update:recipes]"
                                           code = Some 422
                                           errors = List.empty }
                  }
          ...
          destroy =
              ctx.Authorize <| fun recipeId ->
                  async {
                      let userId = Recipes.extractUserId ctx
                      let! result = Recipes.findRecipe recipeId false
                      match result with
                      | Result.Ok recipe ->
                          if recipe.userid <> userId then
                              ctx.HttpContext.Response.StatusCode <- 403
                              ...
                              | Error ex ->
                                  ...
                                  ctx.HttpContext.Response.StatusCode <- 422
                                  return Error
                                             { message = "Unexpected Error in [delete:recipes]"
                                               code = Some 422
                                               errors = List.empty }
                      | Error ex ->
                          eprintfn "%O" ex
                          ctx.HttpContext.Response.StatusCode <- 422
                          return Error
                                     { message = "Unexpected Error in [delete:findRecipe:recipes]"
                                       code = Some 422
                                       errors = List.empty }
                  } }
