...
module private Auth =
    ...
    let findUser (email: string) 
        : Async<Option<{| id: int; name: string; lastName: string; email: string; password: string |}>> =
        async {
            let! result = 
                defaultConnection
                |> Sql.connect
    ...
    let signupUser (payload: {| name: string; lastName: string; email: string; password: string |})
        : Async<Result<User, exn>> =
        async {
            let! result = 
                defaultConnection
                |> Sql.connect
