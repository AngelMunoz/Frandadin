...
module private Ingredients = 
    ...
    let create (ingredient: {| recipeid: int; name: string; quantity: string |})
        : Async<Result<Ingredient, exn>> =
        async {
            let! inserts = 
                defaultConnection
                |> Sql.connect
                ...
                async {
                    ...
                    | Result.Ok access ->
                        ...
                        | true ->
                            ...
                            | ValidationState.Ok ->
                                let! result = Ingredients.create {| recipeid = ingredient.recipeid; name = ingredient.name; quantity = ingredient.quantity |}
                                match result with 
                                | Result.Ok result ->
