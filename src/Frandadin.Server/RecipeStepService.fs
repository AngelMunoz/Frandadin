...
module private RecipeSteps =
    ...
    let create (recipestep: {| recipeid: int; order: int; directions: string; imageUrl: Option<string> |}): Async<Result<RecipeStep, exn>>
        =
        async {
            let! inserts = defaultConnection
                           |> Sql.connect
                           |> Sql.query @"INSERT INTO recipesteps(recipeid, steporder, directions, imageurl)
            ...
            | Result.Ok rows ->
                let query =
                    defaultConnection
                    |> Sql.connect
                    |> Sql.query @"SELECT * FROM recipesteps WHERE id = @id"
